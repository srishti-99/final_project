...
def callback(obstacle): 
	...
	obstacle.dimensions.x += k*(radius_of_robot + radius_of_block)
	obstacle.dimensions.y += k*(radius_of_robot + radius_of_block)

	return obstacle

def listener():
	s = rospy.Service("create_obstacle", Obstacle, callback)
    rospy.spin()
